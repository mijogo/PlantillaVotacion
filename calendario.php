...
if($_GET['action']==0)
{
	...
	$pagina = ingcualpag($pagina,"tabla_objetos_1",tablaobjetos("Calendario",arreglocalendario()));
...
function arreglocalendario()
{
	...
	for($i=0;$i<count($mcalendario);$i++)
	{
		...
		$objetos[] = array($accion,$mcalendario[$i]->getfecha(),$hecho,"<a href=\"modificarcalendario.php?idcalendario=".$mcalendario[$i]->getid()."\">Modificar</a>","<a href=\"calendario.php?action=1&idcalendario=".$mcalendario[$i]->getid()."\">Eliminar</a>");

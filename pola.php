...
$opcion = array();
$valor = array();
for($i=3;$i<count($reto);$i++)
{
	if($i%2==1)
		$opcion[]=$reto[$i];
	else
	{		
		$cambio = explode(">",$reto[$i]);
		$cambio = explode("<",$cambio[1]);
		$valor[]=$cambio[0];
	}	
}
$textoopcion="paisesabr=array(";
$textovalor="paisesnom=array(";
for($i=0;$i<count($opcion);$i++)
{
	...
	$textoopcion.="\"$opcion[$i]\"";
	$textovalor.="\"$valor[$i]\"";
}
$textoopcion.=");";
$textovalor.=");";
echo $textoopcion."<br>".$textovalor;
?>